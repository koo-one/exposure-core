...
export function BadDebtPanel({
  ...
  const copy = {
    realizedDebt: labels?.realizedDebt ?? "Realized Debt",
    coveredDebt: labels?.coveredDebt ?? "Promised / Recovered",
    uncoveredGap: labels?.uncoveredGap ?? "Unresolved",
    recoveryRate: labels?.recoveryRate ?? "Resolution Rate",
  };
