...
export default function AssetPage() {
  ...
  const { terminalToast, showTerminalToast, closeTerminalToast } = useTerminalToast();
  ...
  const handleDrilldownSelect = async (
    ...
    const hasChildren = graphData?.edges.some((e) => e.from === node.id) ?? false;
    if (hasChildren) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-white gap-6">
        ...
        <p className="text-black font-mono text-[10px] uppercase tracking-[0.3em] animate-pulse">Establishing Data Connection</p>
      </div>
    );
  }
    ...
    return (
      <div className="p-8 text-center text-black h-screen flex flex-col items-center justify-center bg-[#E6EBF8]">
        <div className="bg-white p-12 rounded-sm border border-black max-w-md w-full shadow-2xl">
          <h2 className="text-lg font-bold text-black mb-4 tracking-tight uppercase">Registry Access Denied</h2>
          <p className="text-black/60 mb-10 text-xs leading-relaxed font-mono">ID_{id.toUpperCase()} not found in the indexed distribution network.</p>
          <Link href="/" className="inline-flex items-center justify-center w-full px-8 py-4 bg-black text-white font-bold text-[10px] uppercase tracking-[0.2em] rounded-sm hover:bg-black/80 transition-colors">
            Return to Registry
          </Link>
        </div>
      </div>
      ...
      <header className="bg-white border-b border-black px-10 py-6 flex justify-between items-center z-30 flex-shrink-0">
        <div className="flex items-center gap-12">
          ...
          <div className="flex flex-col">
            <h1 className="text-xl font-bold text-black tracking-tight flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 mt-1.5">
               ...
               <span className="text-[9px] font-bold text-black/40 uppercase tracking-[0.2em]">Real-Time Distribution Map</span>
            </div>
          </div>
        </div>
